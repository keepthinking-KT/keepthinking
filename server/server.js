// KeepThinking v7.7.0 — Independent HTTP Server
...
<p class="footer">v7.7.0 · 数据100%本地 · 零上传</p>
</div>
<script>
async function login() {
  ...
  try {
    const r=await fetch((location.pathname.startsWith('/kt')?'/kt':'')+'/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:p})});
    if(r.ok) {
      const d=await r.json();
      document.cookie='kt_token='+d.token+';path='+(location.pathname.startsWith('/kt')?'/kt':'/')+';max-age=86400';
      location.reload();
...
document.getElementById('pwd').addEventListener('keydown',e=>{if(e.key==='Enter')login()});
  ...
  else res.type("html").send("<h1>KeepThinking v7.7.0</h1>");
});
...
app.listen(PORT, HOST, async () => {
  console.log("[keepthinking-server] v7.7.0 — http://" + HOST + ":" + PORT);
